...
// derived from Il2CppInterop.Common/MiniILParser.cs
...
internal static class MiniIlParser
{
    ...
    {
        foreach (
            var fieldInfo in typeof(OpCodes).GetFields(BindingFlags.Static | BindingFlags.Public)
        )
        {
            ...
            switch (argLength)
            {
                ...
                default:
                    throw new NotSupportedException(
                        $"Unsupported opcode argument length {argLength}"
                    );
            }
